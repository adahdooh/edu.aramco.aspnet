...
{
    public static void ApplySMSConfiguration<T>(this ModelBuilder modelBuilder)
        where T : SMS
    {
        ...
        modelBuilder.Entity<T>().Property(s => s.PhoneNumber).IsRequired(true).HasMaxLength(15);

        modelBuilder.Entity<T>().Property(s => s.Body).IsRequired().HasMaxLength(160);
    }
}
